...
public class Application1 {
    //예외에 대해 이해하고 처리하기 위한 문법을 이용 할 수 있다.
    //예외처리
    //자바 프로그램 작성 시 자바 문법에 맞지 않는 경우
    //코드를 컴파일 하려고 할 때 컴파일 에러를 발생시킨다. 엄밀히 말하면 문법상의 오류이다.
    //혹은 자바 문법에 맞게 작성하여 컴파일에 문제가 없더라도, 실행되면서 예상치 못하게 오류가 발생할 수 있다.
    //컴퓨터 프로그램이 동장하는 도중에 예상치 못한 사태가 발생하여
    //실행중인 프로그램이 영향을 받는 것을 오류와 예이 두가지로 구분할 수 있다.

    //오류(Error)
    //시스템 상에서 프로그램에 심각한 물제를 발생하여 실행중인 프로그램이 종료되는 것을 말한다.
